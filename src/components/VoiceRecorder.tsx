...
export type Status = "idle" | "recording" | "recorded" | "denied" | "unsupported";
...
export const VoiceRecorder = forwardRef<VoiceRecorderHandle, Props>(
  function VoiceRecorder(
    { onChange, onRecordingChange, onStatusChange, disabled, hideIdleButton },
    ref
  ) {
    ...
    // If nothing to show (idle + parent is driving start), render nothing.
    if (status === "idle" && hideIdleButton && !error) {
      return null;
    }

    return (
      <div className="space-y-2">
        {status === "idle" && !hideIdleButton && (
          <button
            type="button"
        ...
        {status === "recording" && (
          <div className="flex items-center gap-3">
            <button
              ...
              className="inline-flex items-center gap-2 rounded-full bg-red-600 text-white px-3 py-1.5 text-xs font-medium hover:bg-red-700"
            >
